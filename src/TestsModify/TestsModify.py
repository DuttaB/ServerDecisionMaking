...
class TestServerMethods(unittest.TestCase):
    ...
    def test_sensors(self):
        sensor_events = [
            generate_sensor_json("INSERT", "0"),              # Generates a sensor where value is okay
        ]
        sensor_responses = [
            "0"
        ]
    ...
    def test_lambda(self):
        lambda_events = [
            ...
            generate_lambda("1", "sensors", "MODIFY"),  # no change(old status defaults to fire
            generate_lambda("0", "sensors", "MODIFY"),  # no change(old status defaults to fire
        ]

        lambda_responses = [
            '{"message": {"body": {"status": "1", "xpos": "7", "floor": "6", "ypos": "4", "room": "3"}, "msg_type": "update status"}, "id": ["5"]}',
            '{"message": {"body": {"status": "0", "xpos": "7", "floor": "6", "ypos": "4", "room": "3"}, "msg_type": "update status"}, "id": ["5"]}'
        ]
